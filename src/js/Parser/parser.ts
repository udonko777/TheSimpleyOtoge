import {
  bmsChannelToKeyStatement,
  isConvertibleKeyChannel,
...
const getBMSMainDefinition = (
  measuresIndex: number,
  channel: number,
  notesPositions: ReadonlyArray<string> = [],
): BMSMainDefinition => {
...
const bmsTokenizer: ChartTokenizer<BMSMainDefinition> = (sourceText) => {
  ...
  for (const L of mainDataLines) {
    ...
    for (let i = 0; i + mainDataFields.length < measuresIndex; i++) {
      mainDataFields.push(getBMSMainDefinition(i + mainDataFields.length, 0));
    }
...
const bmsScanner: ChartTokenScanner<BMSMainDefinition> = ([
  mainDataFields,
  countOfMeasures,
]) => {
  //小節の集合の初期化
  ...
  for (const dataField of mainDataFields) {
    ...
    if (!bmsChannelToKeyStatement.has(channel)) {
      throw new Error(
        "BMSChannelToKeyStatement returned an invalid value. chartConfig may be incorrect.",
      );
    }
    ...
    for (const [i, position] of positions.entries()) {
      ...
      measures[measuresIndex].notePositions.set(
        i * beat,
